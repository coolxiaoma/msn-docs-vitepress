...
### iframe
第一步：
在你的 Dify 应用中，前往 **发布 → 嵌入网站** 找到你的唯一令牌。
第二步：
选择一种方式将聊天应用嵌入到你的网站中`(注意ip地址，如果它提供的案例未携带端口自己需要拼一个端口在案例后面，或者看你需要是否是https协议)`
...
在你的 Dify 应用中，前往 **发布 → 嵌入网站** 找到你的唯一令牌。
第二步：
找到你的token和baseUrl，填写YOU_TOKEN和YOU_BASEURL就能正常使用了
`(注意ip地址，如果它提供的案例baseUrl未携带端口自己需要拼一个端口在案例后面，或者看你需要是否是https协议)`
```vue
...
const difyChatbotConfig = {
  ...
  baseUrl: "https://YOU_BASEURL",
  // 关键配置：让脚本立即执行而不是等待body.onload
  dynamicScript: true,
...
const getDifyScriptUrl = (): string => {
  return `https://YOU_BASEURL/embed.min.js?v=${Date.now()}` || "";
};
// 初始化 Dify 聊天机器人
